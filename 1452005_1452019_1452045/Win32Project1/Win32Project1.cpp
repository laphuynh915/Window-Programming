...
COLORREF myColorB = RGB(255, 255, 255);
void OnEditCopy();
void OnEditPaste();
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case ID_DRAW_WIDTH:
			...
			DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG2), hEdit,(DLGPROC) AboutBox);
			break;
		...
		case ID_FORMAT_TEXT:
		{
			...
			UpdateWindow(hWnd);
			//Hàm vẽ chữ			
			TextOut(hdc, 50, 100, buff,sizeof(buff));
			...
			SelectObject(memDC, hBitmap);
			//BitBlt(hdc, 100, 100, 450, 400, memDC, 0, 0, SRCCOPY);

			//DrawText(memDC, buff, -1, &bounds, DT_CENTER | DT_VCENTER);
			TextOut(memDC, 50, 100, buff, sizeof(buff));
			DeleteDC(memDC);
			DeleteObject(memDC);
		...
		case ID_FILE_OPEN:
			...
			if (GetOpenFileName(&ofn) == TRUE) {
				...
				RECT rect;
				GetClientRect(hWnd, &rect);
				BitBlt(hdc,100, 100, rect.right - rect.left, rect.bottom - rect.top, memDC, 0, 0, SRCCOPY);
				DeleteDC(memDC);
			}
		...
		case ID_EDIT_COPY:
			...
			if (OpenClipboard(hWnd))
			{
				...
				GetClientRect(hWnd, &rect);
				HDC memDC = CreateCompatibleDC(hdc);
				HBITMAP junk = CreateCompatibleBitmap(hdc, rect.right - rect.left, rect.bottom - rect.top);
				SelectObject(memDC, junk);
				//Chép vùng màn hình (0,0,200,200)lên DC ảo  
				BitBlt(hdc, 0, 0, rect.right - rect.left, rect.bottom - rect.top, memDC, 0, 0, SRCCOPY);
				SetClipboardData(CF_BITMAP, hWnd);
				CloseClipboard();
...
VOID PEN(HDC hdc, int width, int R, int G, int B)
{
	HPEN  pen = CreatePen(line, width, myColor);
	HBRUSH brush = CreateSolidBrush(myColorB);
...
void doChooseColor(HWND hWnd)
{
	...
	else
	{
		...
		cc.hwndOwner = hWnd;
		cc.lpCustColors = (LPDWORD)acrCustClr;
		cc.rgbResult = myColorB;
		cc.Flags = CC_FULLOPEN | CC_RGBINIT;
		if (ChooseColor(&cc))
		{
			HDC hdc = GetDC(hWnd);
			myColorB = cc.rgbResult;
			SetTextColor(hdc, myColorB);
		}
	}
}
PBITMAPINFO CreateBitmapInfoStruct(HWND hwnd, HBITMAP hBmp)
